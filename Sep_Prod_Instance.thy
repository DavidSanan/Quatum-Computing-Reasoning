...
begin
  definition
  plus_set_def: "m1 + m2 \<equiv> m1 \<union> m2"

definition
  sep_disj_set_def: "sep_disj m1 m2 \<equiv> m1 \<inter> m2 = {}"
instance
  apply standard
  by (auto simp add: sep_disj_set_def  zero_set_def plus_set_def)           
end


instantiation set :: (type) opt
begin
  definition none_def [simp]: "none \<equiv> {}"
  instance ..
end
