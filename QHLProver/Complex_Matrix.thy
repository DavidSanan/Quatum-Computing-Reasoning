...
proof -
  ...
  hence "0 \<le> (inner_prod x x - (inner_prod x y) * cnj (inner_prod x y) / (inner_prod y y)) * (inner_prod y y)"
    using mult_nonneg_nonneg by blast
...
lemma vec_norm_geq_0:
  ...
  using self_cscalar_prod_geq_0[of v]
...
proof (intro conjI impI)
  ...
  with dim show "w \<noteq> 0\<^sub>v (dim_vec w) \<Longrightarrow> v = 0\<^sub>v (dim_vec v) \<Longrightarrow> v \<bullet>c w = vec_norm v * vec_norm w * (v \<bullet>c (1 / vec_norm w \<cdot>\<^sub>v w))" by auto
  {
    ...
    then have cw: "conjugate (1 / vec_norm w) = 1 / vec_norm w" 
      using complex_eq_iff complex_is_Real_iff
      using less_eq_complex_def vec_norm_geq_0 by auto 
    from dim0 have 
...
next
  ...
  proof (simp, rule impI)
    ...
    then have nvge0: "1 / vec_norm v > 0" using complex_is_Real_iff
      by (simp add: less_eq_complex_def order_less_le)
...
proof -
  ...
  then have dimaW: "mat_adjoint W \<in> carrier_mat n n" by auto
  {
    ...
    then have ge0: "vec_norm (ws ! i) * vec_norm (ws ! j) > 0"
  ...
  then have dimA: "A \<in> carrier_mat n n" using n_def[symmetric] by auto
  {
    ...
    next
      ...
      then have cnv: "cnj (vec_norm v) = vec_norm v" 
        using Reals_cnj_iff complex_is_Real_iff
        using nonnegative_complex_is_real vec_norm_geq_0 by presburger
...
proof -
    ...
    using complex_mat_decomposition_to_hermitian[OF dimA] by auto
  {
    ...
    moreover have "inner_prod v (A *\<^sub>v v) \<in> Reals" using pA unfolding positive_def 
      ...
      using complex_is_Real_iff dimvA n_def nonnegative_complex_is_real by presburger
...
proof -
  ...
  have Dii: "\<And>i. i < n \<Longrightarrow> D$$(i, i) = csqrt (B$$(i, i))" using dimD D_def by auto
  {
    ...
    then have "c * cnj c = B$$(i, i)" using c_def c unfolding conjugate_complex_def
...
lemma lowner_le_smult:
  ...
  assumes "c \<ge> 0" "A \<le>\<^sub>L B" "A \<in> carrier_mat n n" "B \<in> carrier_mat n n"
  shows "c \<cdot>\<^sub>m A \<le>\<^sub>L c \<cdot>\<^sub>m B"
  apply (rule lowner_le_smultc) using assms by (auto simp add: less_eq_complex_def)
...
proof
  ...
  proof (rule, simp add: carrier_matD(2)[OF dimov] dimov, rule allI, rule impI)
    ...
    also have "\<dots> \<ge> 0" by (auto simp add: less_eq_complex_def)
    ...
    proof -
      {
        ...
        then have dimv: "v \<in> carrier_vec n" and nv: "vec_norm v = 1"
          using carrier_matD[OF dimBmA] carrier_vecI by (auto intro: carrier_vecI)
        have dimov: "outer_prod v v \<in> carrier_mat n n" using dimv by auto
