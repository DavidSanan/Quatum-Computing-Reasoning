...
proof (rule ccontr)
  ...
  proof -
    ...
    ultimately have "inner_prod v v > 0" by (auto simp: less_complex_def)
    then show ?thesis by auto
  qed
  ...
  then have "\<exists>no. \<forall>n\<ge>no. cmod (inner_prod v (X n *\<^sub>v v) - inner_prod v (A *\<^sub>v v)) < -?r" 
    using rl by (auto simp:  less_complex_def)
  ...
  then have "cmod(inner_prod v (X N *\<^sub>v v)) - cmod(inner_prod v (A *\<^sub>v v)) < cmod(inner_prod v (A *\<^sub>v v))" 
    using rl cmod_eq_Re by (auto simp:  less_complex_def)
  then have  "cmod(inner_prod v (X N *\<^sub>v v)) < 0"  
    using XNv XX YY cmod_eq_Re by (auto simp: less_eq_complex_def less_complex_def)
...
proof-
  ...
  moreover from trless tracefn  have "norm(trace (f n)) \<le> norm(trace (f (Suc n)))" 
    unfolding cmod_def by (simp add: less_eq_complex_def)
  moreover from trless tracefn have "norm(trace (f n)) \<le> 1" 
    using pdo partial_density_operator_def cmod_def by (simp add: less_eq_complex_def less_complex_def)
    ultimately show ?thesis by auto
  qed
  ...
  moreover have "\<forall>m n. dist(norm(trace (f m))) (norm(trace (f n))) = norm(trace (f m) - trace (f n))"
    using tracefn cmod_eq_Re dist_real_def 
    by (simp add: less_eq_complex_def)
  ...
  proof -
    ...
    finally show ?thesis by (simp add: less_eq_complex_def)
  qed
...
proof -
  have "\<forall> n. trace (f n) \<ge> 0" using positive_trace pdo unfolding partial_density_operator_def
    using dim by blast
  then have Re: "\<forall> n. Re (trace (f n)) \<ge> 0 \<and> Im (trace (f n)) = 0" by (simp add: less_eq_complex_def)
  then have lex: "\<forall> n. Re (trace (f n)) \<le> Re x \<and> Im x = 0" using assms by (simp add: less_eq_complex_def)
  ...
  from Rell Imll lex show ?thesis by (simp add: less_eq_complex_def)
qed
...
proof -
  ...
  then have "trace (A - f 0) + 1 > 0" by (simp add: less_eq_complex_def less_complex_def)
  then have gtc: "c > 0" unfolding c_def using complex_is_Real_iff by (simp add:  less_complex_def)
  then have gtci: "(1 / c) > 0" using complex_is_Real_iff by (simp add: less_complex_def)
  ...
  then have trlt1: "trace (c \<cdot>\<^sub>m (f n - f 0)) < 1" for n 
    using lt1 unfolding less_eq_complex_def less_complex_def
  ...
  then have ms: "matrix_seq d g" 
    unfolding matrix_seq_def partial_density_operator_def 
