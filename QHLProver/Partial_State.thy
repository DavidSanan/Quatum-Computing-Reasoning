...
lemma valid_index_ind_in_set:
  assumes "is \<lhd> nths dims1 A" "B \<subseteq> A"
  shows "nths is (ind_in_set A ` B) \<lhd> nths dims1 B"
...
lemma cong_vec: assumes "ne = ne'" "\<And> i j. i < ne \<Longrightarrow>
  f i = f' i" shows "Matrix.vec ne f = Matrix.vec ne' f'"
  by (rule eq_vecI, insert assms, auto)

lemma card_A_B_ind_in_set_A:
     assumes a0:"A \<inter> B = {}" and a1:"finite (A::nat set)" and a2:"finite B" 
     shows "card {i. i < length (replicate (card (A \<union> B)) 2) \<and> i \<in> ind_in_set (A \<union> B) ` A} = card A"
proof-
  have h1:"card (ind_in_set (A \<union> B) ` A) = card A"     
    by (simp add: card_image ind_in_set_inj')
  have h2:"ind_in_set (A \<union> B) ` A \<subseteq> {0..<(card A + card B)}"
    by (metis card_Un_disjoint a0 finite_UnI a1 a2 image_mono ind_in_set_id sup_ge1)
  have " {i. i < length (replicate (card (A \<union> B)) 2) \<and> i \<in> ind_in_set (A \<union> B) ` A} = 
           {0..< card A + card B} \<inter>  ind_in_set (A \<union> B) ` A"
    using h1 h2  a1 a2 a0 by (auto simp add: card_Un_disjoint)
  then show ?thesis using h1 h2 by (simp add: Int_absorb2 Int_commute)
qed
...
lemma scalar_tensor_1:
  assumes a0:"d1 \<le> dim_vec v1" and a1:"d2 \<le> dim_vec v2" 
  shows "(a::'a::field) \<cdot>\<^sub>v (tensor_vec v1 v2) = tensor_vec  (a \<cdot>\<^sub>v v1) v2"
proof-
  have st1len:"dim_vec (tensor_vec v1 v2) = d"
    unfolding tensor_vec_def  
    by (simp add: a0 state_sig.d_def)
  then have decode_ptensor:
    "\<And>v1 v2. \<forall>i < d. (tensor_vec v1 v2) $ i = 
                        v1 $ (encode1 i) * v2 $ (encode2 i)"
    by (simp add:  partial_state.tensor_vec_eval)
  {  
    fix i
    assume a00:"(i::nat) < d"
    then have "((a::'a::field) \<cdot>\<^sub>v (tensor_vec v1 v2)) $ i = 
               a * (v1 $ (encode1 i) * 
                   v2 $ (encode2 i))"
      using  st1len by (simp add: decode_ptensor)
  }
  moreover {
    fix i
    assume a00:"(i::nat) < d"
    then have d:"i<d" using a00
      using st1len by fastforce
    have "(tensor_vec  v1 v2) $ i =  (v1 $ (encode1 i) * v2 $ (encode2 i))"
      using  a00 
      by (simp add: decode_ptensor mult.assoc)
    have "encode1 i < dim_vec v1"
      using  d a0 partial_state.encode1_lt
      using order_less_le_trans by blast
    then have "(tensor_vec  (a \<cdot>\<^sub>v v1) v2) $ i = 
               a * (v1 $ (encode1 i) * 
                   v2 $ (encode2 i))"
      using  a00 
      by (simp add: decode_ptensor mult.assoc)
...
lemma scalar_tensor_2:
  assumes a0:"d1 \<le> dim_vec v1" and a1:"d2 \<le> dim_vec v2" 
  shows "(a::'a::field) \<cdot>\<^sub>v (tensor_vec v1 v2) = tensor_vec  v1 (a \<cdot>\<^sub>v v2)"
proof-
  have st1len:"dim_vec (tensor_vec v1 v2) = d"
    unfolding tensor_vec_def  
    by (simp add: a0 state_sig.d_def)
  ...
  moreover {
    ...
    have "(tensor_vec  v1 v2) $ i =  (v1 $ (encode1 i) * v2 $ (encode2 i))"
      using  a00 
      by (simp add: decode_ptensor mult.assoc)
    have "encode2 i < dim_vec v2"
      using  d a1 partial_state.encode2_lt
      using order_less_le_trans  by blast
    then have "(tensor_vec v1 (a \<cdot>\<^sub>v v2)) $ i = 
               a * (v1 $ (encode1 i) * 
                   v2 $ (encode2 i))"
      using  a00 
      by (simp add: decode_ptensor mult.assoc)
...
lemma scalar_tensor:
  assumes a0:"d1 \<le> dim_vec v1" and a1:"d2 \<le> dim_vec v2" 
  shows "((a::'a::field)*b) \<cdot>\<^sub>v (tensor_vec v1 v2) = tensor_vec  (a \<cdot>\<^sub>v v1) (b \<cdot>\<^sub>v v2)"
  by (smt (verit, ccfv_SIG) a0 a1 index_smult_vec(2) scalar_tensor_1 scalar_tensor_2 smult_smult_assoc)
...
definition dims0 :: "nat list" where
  "dims0 = replicate (card vars0) 2"

definition dims1 :: "nat list" where
  "dims1 = replicate (card vars1) 2"

definition dims2 :: "nat list" where
  "dims2 = replicate (card vars2) 2"
...
lemma "finite vars1 \<Longrightarrow> finite vars2 \<Longrightarrow> vars1 \<inter> vars2 = {} \<Longrightarrow> card (vars1 \<union> vars2) = card vars1 + card vars2"
  using card_Un_disjoint by blast

lemma dims_product:
  "d0 = d1 * d2"
  unfolding d0_def d1_def d2_def dims0_def dims1_def dims2_def vars0_def
  using  disjoint finite_v1 finite_v2 apply auto
...
lemma ind_in_set_bij:
  "bij_betw (ind_in_set vars0) ({0..<length dims0} \<inter> vars0) {0..<card ({0..<length dims0} \<inter> vars0)}"
  using bij_ind_in_set_bound unfolding ind_in_set_def by auto

lemma length_dims0:
  "length dims0 = card vars1 + card vars2"
  unfolding dims0_def 
  by (simp add: card_Un_disjoint disjoint finite_v1 finite_v2 vars0_def)
...
proof -
  ...
  show ?thesis unfolding vars2'_def vars1'_def unfolding dims0_def  
    using ind_in_set_minus_subset_bound[OF sub20] 
            sub1 finite_v1 finite_v2 disjoint apply auto
    apply (metis Un_iff atLeast0LessThan finite_UnI image_Un ind_in_set_id lessThan_iff vars0_def)
    by (metis DiffD1 disjoint_iff_not_equal ind_in_set_inj sub20 subset_eq)
qed
...
proof -
  ...
  show ?thesis unfolding vars2'_def vars1'_def unfolding dims0_def 
    using ind_in_set_minus_subset_bound[OF sub10] sub2 apply auto
     apply (metis Un_iff atLeast0LessThan finite_UnI finite_v1 finite_v2 image_Un ind_in_set_id lessThan_iff vars0_def)
    by (metis DiffD1 disjoint disjoint_iff_not_equal ind_in_set_inj sub10 subset_eq)
qed
...
lemma nths_vars1':
  ...
  using card_A_B_ind_in_set_A[OF disjoint finite_v1 finite_v2]  unfolding vars0_def  vars1'_def dims1_def dims0_def        
    using finite_v1 finite_v2 
           length_nths[of "replicate (card (vars1 \<union> vars2)) 2" "ind_in_set (vars1 \<union> vars2) ` vars1"]
    by (simp add: nths_replicate)   

lemma nths_vars1'_comp:
  "nths dims0 (-vars2') = dims1" 
  using nths_vars1' disjoint[simplified Int_commute[of vars1 vars2]]
  unfolding vars1'_def vars2'_def 
  apply auto
  apply (frule nths_complement_ind_in_set[of vars2 vars1 dims0])     
  unfolding vars0_def[simplified Un_commute[of vars1 vars2]] dims0_def
  by auto 

lemma nths_vars2'_comp:
  "nths dims0 (vars2') = dims2" 
  using card_A_B_ind_in_set_A[OF disjoint[simplified Int_commute[of vars1 vars2]] finite_v2 finite_v1,simplified Un_commute] 
  unfolding vars0_def  vars2'_def dims2_def dims0_def        
    using finite_v1 finite_v2 
           length_nths[of "replicate (card (vars1 \<union> vars2)) 2" "ind_in_set (vars1 \<union> vars2) ` vars2"]
    by (simp add: nths_replicate)

lemma nths_vars2':
  "nths dims0 (-vars1') = dims2" 
 using nths_vars2'_comp disjoint[simplified Int_commute[of vars1 vars2]]
  unfolding vars1'_def vars2'_def 
  apply auto
  apply (frule nths_complement_ind_in_set[of vars2 vars1 dims0])      
  unfolding vars0_def[simplified] dims0_def  
   by (auto simp add: disjoint nths_complement_ind_in_set sup_commute) 
  
  

(* lemma nths_vars1':
  "nths dims0 vars1' = dims1" 
  using nths_reencode_eq[of vars1 vars0 dims0] 
    using nths_reencode_eq_comp[of vars1 vars0 dims0] 
...
definition pencode12::"nat\<times>nat \<Rightarrow> nat" where
  "pencode12 p12 \<equiv> partial_state.encode12 dims0 vars1' p12"
...
lemma ptensor_vec_comm:
  fixes v1 v2 :: "'a::comm_ring_1 vec"
...
proof -
  ...
  show ?thesis using eq1 eq2 
    unfolding st1.ptensor_vec_def st2.ptensor_vec_def partial_state.tensor_vec_def 
    unfolding st1.dims0_def st1.vars0_def 
            st2.dims0_def st2.vars0_def apply auto
    by (simp add: mult.commute sup_commute)
qed


lemma ptensor_mat_comm:
  fixes m1 m2 :: "'a::comm_ring_1 mat"
  assumes "vars1 \<inter> vars2 = {}" and "finite vars1" and "finite vars2"
...
proof -
  ...
  (* have **: "{i. i < length dims \<and> (i \<in> vars1 \<or> i \<in> vars2 \<or> i \<in> vars3)} = vars1 \<union> vars2 \<union> vars3"
    using assms f_v1 f_v2 f_v3 by force *)

  have finite_union: "finite (vars1 \<union> vars2 \<union> vars3)"
    using f_v1 f_v2 f_v3
    using subset_eq_atLeast0_lessThan_finite by blast
  ...
  have m1eq: "digit_encode a.dims0 (digit_decode b.dims1 (nths (digit_encode b.dims0 i) b.vars1')) 
    ...
    unfolding a.dims0_def a.vars0_def b.dims1_def 
    apply (subst digit_encode_decode, auto)   
    using b.nths_vars1'  unfolding  b.dims1_def    
    by (metis digit_encode_valid_index state_sig.d_def that valid_index_nths)            
  ...
  have m2eq1: "digit_encode a.dims0 (digit_decode (nths b.dims0 b.vars1') (nths (digit_encode b.dims0 i) b.vars1'))
    ...
    apply (subst digit_encode_decode, auto)    
    by (metis b.dims1_def b.nths_vars1'
                  digit_encode_valid_index state_sig.d_def that valid_index_nths)    
  ...
  have m2eq2: "digit_encode c.dims0 (digit_decode (nths d.dims0 (- d.vars1')) (nths (digit_encode d.dims0 i) (- d.vars1')))
    ...
    by (metis b.dims0_def b.vars0_def d.dims0_def d.dims2_def d.nths_vars2'_comp 
                 d.vars0_def d.vars2'_def digit_encode_valid_index state_sig.d_def 
                 that uassoc valid_index_nths)             
  ...
  have m2index: "partial_state.encode2 a.dims0 a.vars1' (partial_state.encode1 b.dims0 b.vars1' i) =
    ...
    apply (rule arg_cong[where f="digit_decode (replicate (card vars2) 2)"])
    ...
    subgoal apply (auto simp only: length_digit_encode length_nths)
      by (auto simp add:  uassoc)
    using  uassoc
    by (simp add: nths_assoc_three_B[OF assms(1,2)]) 
  ...
  have m3eq: "digit_encode c.dims0 (digit_decode d.dims2 (nths (digit_encode d.dims0 i) (- d.vars1')))
    ...
    apply (subst digit_encode_decode, auto)
     ...
     subgoal apply (auto simp only: length_digit_encode length_nths)
      by (auto simp add:  uassoc)
    by (metis b.dims0_def b.vars0_def d.dims0_def d.dims2_def 
                 d.nths_vars2'_comp d.vars0_def d.vars2'_def 
                 digit_encode_valid_index state_sig.d_def that uassoc valid_index_nths)
  ...
  show ?thesis
    unfolding a.ptensor_vec_def b.ptensor_vec_def c.ptensor_vec_def d.ptensor_vec_def partial_state.tensor_vec_def       
    apply (rule cong_vec)  
    ...
    subgoal for i
    proof -
      assume lti: "i < state_sig.d b.dims0"        
      then have lti':"i < state_sig.d d.dims0"
        by (simp add: b.dims0_def b.vars0_def d.dims0_def d.vars0_def uassoc)
      ...
      have lt1: "partial_state.encode2 d.dims0 d.vars1' i < state_sig.d c.dims0"
        using partial_state.encode2_lt[OF lti, where vars=d.vars1'] eq1
        by (simp add: b.dims0_def b.vars0_def d.dims0_def d.vars0_def uassoc)
      have lt3: "partial_state.encode1 b.dims0 b.vars1' i < state_sig.d a.dims0"
        using partial_state.encode1_lt[OF lti, where vars=b.vars1'] eq2
        by (simp add: b.dims0_def b.vars0_def d.dims0_def d.vars0_def uassoc)     
      show ?thesis               
        apply (auto simp add: lt1  lt3)
        apply (auto simp only: m1index[OF lti]  m2index[OF lti] m3index[OF lti])
        by auto
    qed
    done
qed
...
lemma pmat_extension_assoc:
  assumes "vars1 \<inter> vars2 = {}" 
    and "(vars1 \<union> vars2) \<inter> vars3 = {}" and "finite vars1" and "finite vars2" and "finite vars3"    
...
value "prod_list (replicate (card ({}::(nat set))) (2::nat)) "

lemma pscalar_ptensor_1:
  ...
  shows "(a::'a::field) \<cdot>\<^sub>v (ptensor_vec vs1 vs2 v1 v2) =
    ptensor_vec vs1 vs2 (a \<cdot>\<^sub>v v1) v2"
proof-
  ...
  moreover {
    ...
    then have "(a \<cdot>\<^sub>v v1) $ (partial_state.encode1 st1.dims0 st1.vars1' i) = a *(v1 $ (partial_state.encode1 st1.dims0 st1.vars1' i))"
      by auto
    then have "(ptensor_vec vs1 vs2 (a \<cdot>\<^sub>v v1) v2) $ i = 
               a * (v1 $ (partial_state.encode1 st1.dims0 st1.vars1' i) * 
                   v2 $ (partial_state.encode2 st1.dims0 st1.vars1' i))"
      using  a00 
      by (simp add: decode_ptensor mult.assoc)
...
lemma pscalar_ptensor:
  assumes a0:"vs1 \<inter> vs2 = {}" and a1:"finite vs1" and a2:"finite vs2" and
  a3:"dim_vec v1 = 2^card vs1" and a4:"dim_vec v2 = 2^card vs2"
  shows "((a::'a::field) * b) \<cdot>\<^sub>v (ptensor_vec vs1 vs2 v1 v2) =
    ptensor_vec vs1 vs2 (a \<cdot>\<^sub>v v1) (b \<cdot>\<^sub>v v2)"
proof -
  have f1: "a * b \<cdot>\<^sub>v ptensor_vec vs1 vs2 v1 v2 = a \<cdot>\<^sub>v (b \<cdot>\<^sub>v ptensor_vec vs1 vs2 v1 v2)"
    by (simp add: smult_smult_assoc)
  have f2: "{} = vs1 \<inter> vs2 \<and> finite vs1 \<and> finite vs2 \<and> dim_vec v1 = 2 ^ card vs1 \<and> dim_vec v2 = 2 ^ card vs2"
    using a0 a1 a2 a3 a4 by presburger
  have f3: "2 ^ card vs2 = dim_vec (b \<cdot>\<^sub>v v2)"
    using a4 by fastforce
  have f4: "b \<cdot>\<^sub>v ptensor_vec vs1 vs2 v1 v2 = ptensor_vec vs1 vs2 v1 (b \<cdot>\<^sub>v v2)"
    using f2 by (simp add: pscalar_ptensor_2)
  have "ptensor_vec vs1 vs2 (a \<cdot>\<^sub>v v1) (b \<cdot>\<^sub>v v2) = a \<cdot>\<^sub>v ptensor_vec vs1 vs2 v1 (b \<cdot>\<^sub>v v2)"
