...
datatype ('a, 's) com = 
  ...
  | Alloc "'a"  "('s,complex list) expr"  ("_:=alloc (_)" [60, 61] 60)
...
lemma new_q_addr_empty_vector:
  assumes a0:"S \<in> new_q_addr f \<sigma> m" and
          a1:"f \<sigma> = []"
        shows "S = {}"
  using a0 a1 unfolding new_q_addr_def by auto
